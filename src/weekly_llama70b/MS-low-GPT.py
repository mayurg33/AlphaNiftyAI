...
BENCHMARK = "NSEI"
CONFIDENCE_THRESHOLD = 7  # High GPT = 7 or above
...
def backtest_high_gpt():
    ...
    for week in weeks:
        ...
        for filename in os.listdir(week_path):
            ...
            path = os.path.join(week_path, filename)

            try:
                with open(path) as f:
                    data = json.load(f)
                    signal = data.get("decision", "").strip().upper()
                    score = data.get("confidence", 0)
                    if signal == "BUY" and score <= CONFIDENCE_THRESHOLD:
    ...
    df.to_csv("results/backtests/high_gpt.csv", index=False)
    with open("results/backtests/high_gpt_portfolio.json", "w") as f:
