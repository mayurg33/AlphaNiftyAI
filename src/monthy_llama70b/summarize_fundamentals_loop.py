# src/summarize_fundamentals_loop.py
import os
import json
from groq import Groq
from datetime import datetime, timedelta

FUNDAMENTALS_BASE_DIR = "data/fundamentals"
SUMMARY_BASE_DIR = "results/summaries_fundamentals"
os.makedirs(SUMMARY_BASE_DIR, exist_ok=True)

client = Groq(api_key="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")

def extract_summary_text(fund):
    m = fund.get("metrics", {})
    fields = [
        f"Company: {m.get('companyName')}",
        f"Sector: {m.get('sector')}",
        f"Industry: {m.get('industry')}",
        f"Market Cap: {m.get('marketCap')}",
        f"Trailing PE: {m.get('trailingPE')}",
        f"Forward PE: {m.get('forwardPE')}",
        f"ROE: {m.get('returnOnEquity')}",
        f"Profit Margin: {m.get('profitMargins')}",
        f"Revenue Growth: {m.get('revenueGrowth')}",
        f"Earnings Growth: {m.get('earningsGrowth')}",
        f"Debt/Equity: {m.get('debtToEquity')}"
    ]
    return "\n".join(fields)

def summarize_fundamentals_for_month(month_str):
    month_dir = os.path.join(FUNDAMENTALS_BASE_DIR, month_str)
    output_dir = os.path.join(SUMMARY_BASE_DIR, month_str)
    os.makedirs(output_dir, exist_ok=True)

    for file in os.listdir(month_dir):
        if not file.endswith(f"_{month_str}.json"):
            continue
        try:
            with open(os.path.join(month_dir, file)) as f:
                data = json.load(f)
            text = extract_summary_text(data)
            symbol = data['metrics'].get("symbol", file.split("_")[0])
            prompt = f"You are a financial analyst. Based on the following data, would you recommend BUY, HOLD, or SELL for the stock? Justify briefly.\n\n{text}.Use chain of thoughts to think."
            response = client.chat.completions.create(
                model="llama3-70b-8192",
                messages=[{"role": "user", "content": prompt}]
            )
            summary = response.choices[0].message.content.strip()
            with open(os.path.join(output_dir, f"{symbol}_fundamentals.txt"), "w") as out:
                out.write(summary)
            print(f"[OK] Summarized fundamentals for {symbol} - {month_str}")
        except Exception as e:
            print(f"[ERROR] Summarizing {file}: {e}")

def summarize_fundamentals_all_months():
    start_date = datetime(2025,5, 1)
    months = [(start_date + timedelta(days=30 * i)).strftime("%Y-%m") for i in range(15)]
    for month in months:
        summarize_fundamentals_for_month(month)

# To run for all 15 months:
summarize_fundamentals_all_months()
