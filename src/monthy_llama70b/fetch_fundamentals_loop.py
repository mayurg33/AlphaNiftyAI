# src/fetch_fundamentals_loop.py
import os
import json
...
def fetch_all_fundamentals_for_month(month_str):
    output_dir = f"data/fundamentals/{month_str}"
    os.makedirs(output_dir, exist_ok=True)
    for ticker in nifty_tickers:
        try:
            print(f"[Fetching Fundamentals] {ticker} for {month_str}")
            ...
            with open(f"{output_dir}/{ticker}_{month_str}.json", 'w') as f:
                json.dump(fundamentals, f, indent=2)
        except Exception as e:
            print(f"[ERROR] {ticker}: {e}")

def fetch_fundamentals_all_months():
    start_date = datetime(2024, 5, 1)
    months = [(start_date + timedelta(days=30 * i)).strftime("%Y-%m") for i in range(15)]
    for month in months:
        fetch_all_fundamentals_for_month(month)

# To fetch fundamentals for all 15 months:
fetch_fundamentals_all_months()
